...
import {
  generateItem as makeItem,
  generateBufferItem as cb,
  generateBufferItems,
  generateItem
} from './misc/items';
import {
...
const makeBuffer = (params: BufferParams): Buffer<Data> => {
  ...
  generateBufferItems(_minCache, _maxCache - _minCache + 1).forEach(item =>
...
const checkUpdate =
  ({ min, max, predicate, fixRight, list }: BufferUpdateConfig) =>
  () => {
...
const checkIndexTrackingOnUpdate = (params: BufferUpdateTrackConfig) => () => {
  ...
  const dump =
    debug && JSON.parse(JSON.stringify(buffer.items.map(i => i.get())));
  const absMinBefore = debug && buffer.finiteAbsMinIndex,
    absMaxBefore = debug && buffer.finiteAbsMaxIndex;
  const minBefore = debug && buffer.minIndex,
    maxBefore = debug && buffer.maxIndex;
...
const checkInsert = (params: BufferInsertConfig) => () => {
  ...
  buffer.items.forEach((item, i) => {
    const $index = result
      ? Number(Object.keys(result.list[i])[0])
      : bufferBefore.items[i].$index;
    const id = result ? result.list[i][$index] : bufferBefore.items[i].data.id;
    expect(item.$index).toBe($index);
    expect(item.data.id).toBe(id);
  });

  expect(buffer.absMinIndex).toEqual(
    result ? result.absMin : bufferBefore.absMinIndex
  );
  expect(buffer.absMaxIndex).toEqual(
    result ? result.absMax : bufferBefore.absMaxIndex
  );
};
...
describe('Buffer Spec', () => {
  describe('Update', () =>
    [
      ...
      },
      {
        ...
        fixRight: false,
        predicate: ({ $index }) =>
          $index === 2 ? [makeItem(97), makeItem(98), makeItem(99)] : true,
      ...
      },
      {
        ...
        predicate: ({ $index, data }) => {
          switch ($index) {
            case 1:
              return [makeItem(0), data];
            case 2:
              return [];
            case 3:
              return [makeItem(2), makeItem(4)];
            case 4:
              return [];
            case 5:
              return [data, makeItem(6)];
          }
        },
      ...
      it(config.title, checkUpdate(config as BufferUpdateConfig))
    ));
  ...
  describe('Index tracking on Update', () =>
    [
      ...
      },
      {
        ...
        result: 7,
        predicate: ({ $index, data }) =>
          $index === 2 ? [data, makeItem('x'), makeItem('y')] : true
      },
      {
      ...
      },
      {
        ...
        result: 5,
        predicate: ({ $index, data }) =>
          $index === 7 ? [data, makeItem('x'), makeItem('y')] : true
      },
      {
      ...
      },
      {
        ...
        result: 6,
        predicate: ({ $index, data }) =>
          $index === 5 ? [makeItem('x'), data, makeItem('y')] : true
      },
      {
      ...
      },
      {
        ...
        result: 4,
        predicate: ({ $index }) =>
          $index < 4 || $index > 6 ? true : $index === 5 ? [makeItem('x')] : []
      },
      {
      ...
      },
      {
        ...
        result: 5,
        predicate: ({ $index }) =>
          $index === 5 ? [makeItem('x')] : !($index > 5 && $index <= 8)
      },
      {
      ...
      },
      {
        ...
        result: 2,
        predicate: ({ $index }) =>
          $index === 5 ? [makeItem('x')] : !($index < 5 && $index >= 2)
      },
      {
  ...
  describe('Insert', () =>
    [
      ...
      .forEach(config =>
        it(config.title, checkInsert(config as BufferInsertConfig))
      ));
});
